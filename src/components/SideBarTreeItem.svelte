	...
	{#each items as item, idx}
		...
		<div class="tree-item">
			<div
				...
				class="parent-item p-2 flex flex-row text-white border-b-[1px] border-solid border-[#1E2D3D]"
			>
				<ArrowDropRightFillSYSTEM
					class="transition-transform duration-300 {activeStatusList[currentIdx]
						? 'rotate-90'
						: ''}"
				/>
						...
						<div class="item folder-tree flex flex-col">
							...
							>
								{#if itemsLevel2.type === 'folder'}
									<ArrowRightSLineSYSTEM
										class="transition-transform duration-300 {activeStatusList[
											currentIdx * 10 + idxl2
										]
											? 'rotate-90'
											: ''}"
								...
								<span
									...
									class="{itemsLevel2.type === 'folder' ? 'cursor-pointer' : ''} hover:text-white"
									>{itemsLevel2.name}</span
								>
							</div>
							{#if itemsLevel2.childs !== undefined}
								<div
									class="list-child-level-2 pb-2 flex transition-[max-height opacity] duration-300 flex-col {activeStatusList[
										currentIdx * 10 + idxl2
									]
